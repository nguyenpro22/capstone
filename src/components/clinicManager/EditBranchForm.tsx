...
export default function EditBranchForm({
  ...
  const [selectedOperatingLicense, setSelectedOperatingLicense] =
    useState<File | null>(null);
  const [operatingLicenseExpiryDate, setOperatingLicenseExpiryDate] =
    useState<string>(() => {
  ...
  const handleOperatingLicenseChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    if (e.target.files && e.target.files.length > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.workingTimeStart) {
      formDataToSend.append("workingTimeStart", formData.workingTimeStart);
    }
    if (formData.workingTimeEnd) {
      formDataToSend.append("workingTimeEnd", formData.workingTimeEnd);
    }
  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 dark:bg-black/70 backdrop-blur-sm z-50 p-4">
      ...
      <motion.div
        ...
        className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl dark:shadow-black/30 w-full max-w-2xl flex flex-col overflow-hidden"
                  ...
                  <div className="flex items-center h-[50px] px-4 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800">
                    <label className="flex items-center space-x-3 cursor-pointer">
                      <div className="relative">
                        ...
                        ></div>
                        <div
                          className={`absolute left-0.5 top-0.5 bg-white dark:bg-gray-200 w-5 h-5 rounded-full transition-transform transform ${
                            formData.isActivated ? "translate-x-5" : ""
                          }`}
                        ></div>
                      </div>
                      <span
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative">
                      ...
                      />
                      <Clock className="absolute left-3 top-[calc(50%+0.25rem)] -translate-y-1/2 h-5 w-5 text-gray-400 dark:text-gray-500" />
                    </div>
                    <div className="relative">
                      <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                        {t("workingTimeEnd")}
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <option value="">
                        {addressDetail.provinceName || t("selectProvinceCity")}
                      </option>
                      {isLoadingProvinces ? (
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setOperatingLicenseExpiryDate(getCurrentDateTime())
                        }
