...
import { TransactionalRequest } from '../types';
...
export const walletTransactionCommandApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateWalletTransaction: builder.mutation<any, { id: string; data: FormData }>({
      query: ({ id, data }) => ({
       ...
       any,
       { withdrawalId: string; isApproved: boolean; rejectionReason?: string; isRefetch?: boolean }
     >({
       query: ({ withdrawalId, isApproved, rejectionReason = "", isRefetch = false }) => ({
...
export const { useGetWalletTransactionsQuery, useGetWalletTransactionsForClinicQuery,
  useGetWalletTransactionsForBranchQuery,
 } = walletTransactionQueryApi;
export const { useCreateWalletTransactionMutation,
