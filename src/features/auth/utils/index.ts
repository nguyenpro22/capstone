    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        try {
          ...
          console.warn("Firebase sync warning (không ảnh hưởng đến đăng nhập):", firebaseError)
        }
      ...
      try {
        ...
        if (firebaseError.code === "auth/invalid-credential" || firebaseError.code === "auth/wrong-password") {
          errorMessage = t("invalidCredentials") || "Thông tin đăng nhập không hợp lệ"
...
export const handleGoogleLogin = async ({
  ...
  try {
      ...
      try {
        ...
        if (error && error.data) {
          ...
          return {
            success: false,
            error:
              errorData.detail || t("providerLoginError", { provider: "Google" }) || "Đăng nhập với Google thất bại",
            errorData: errorData,
          }
        }

        const errorMessage = t("providerLoginError", { provider: "Google" }) || "Đăng nhập với Google thất bại"
        showError(errorMessage)
        return { success: false, error: errorMessage }
      }
    } else {
    ...
    let errorMessage = t("providerLoginError", { provider: "Google" }) || "Đăng nhập với Google th���t bại"
...
export const handleLogout = async ({
  ...
  try {
    ...
    try {
      ...
      console.warn("API logout error (không ảnh hưởng đến đăng xuất):", apiError)
    }
...
const processAuthSuccess = async ({
  ...
  try {
    if (provider) {
      showSuccess(
        t("providerLoginSuccess", { provider, userName: userName || "" }) ||
          `Đăng nhập thành công với ${provider}${userName ? ` (${userName})` : ""}`,
      )
    } else {
    ...
    showSuccess(
      provider ? `Đăng nhập thành công với ${provider}${userName ? ` (${userName})` : ""}` : "Đăng nhập thành công",
    )
  }
