...
export default function Category() {
  ...
  >({});
  const [showMoveSubCategoryModal, setShowMoveSubCategoryModal] =
    useState(false);
  const [subCategoryToMove, setSubCategoryToMove] =
    ...
    if (!expandedCategories[categoryId]) {
      if (!categoryDetails[categoryId]) {
        try {
          ...
          toast.error(t("notifications.loadCategoryFailed"));
        }
      }
    }
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    if (window.confirm(t("confirmations.deleteCategory"))) {
      try {
  ...
  const handleDeleteSubCategory = async (subCategoryId: string) => {
    if (window.confirm(t("confirmations.deleteSubcategory"))) {
      try {
        await deleteSubCategory(subCategoryId).unwrap();
        toast.success(t("notifications.subcategoryDeleted"));
        // Use delayed refetch instead of immediate refetch
        delayedRefetch();

        // Cập nhật lại categoryDetails sau khi xóa
        const parentId = Object.keys(categoryDetails).find((key) =>
          categoryDetails[key].subCategories?.some(
        ...
        console.error(error);
        toast.error(t("notifications.subcategoryDeleteFailed"));
      }
    }
  };
  ...
  const handleMoveSubCategory = (subCategory: SubCategory) => {
    ...
    if (!subCategory || !subCategory.id) {
      toast.error(t("notifications.subcategoryMoveFailed"));
      return;
    }
  ...
  const handleMoveSubCategorySubmit = async (destinationCategoryId: string) => {
    ...
    try {
      ...
      toast.success(t("notifications.subcategoryMoved"));
      ...
      if (expandedCategories[destinationCategoryId]) {
        const destResult = await fetchCategoryById(
      ...
      <div className="bg-white dark:bg-gray-800 p-6 shadow-md dark:shadow-gray-900/30 rounded-lg relative">
        ...
        {error && (
          <div className="py-12 text-center">
            ...
            <p className="text-red-500 dark:text-red-400">{t("loadError")}</p>
          </div>
        )}
                      ...
                      <td className="p-3 border border-gray-200 dark:border-gray-700 relative">
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            title={t("actions.addSubcategory")}
                          >
                          ...
                          <button
                            onClick={() =>
                              handleMenuAction("edit", category.id)
                            }
                      ...
                      {expandedCategories[category.id] && (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {/* Subcategories table */}
                              {(categoryDetails[category.id]?.subCategories
                                ?.length ?? 0) > 0 ? (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <CategoryForm
            ...
            onSaveSuccess={() => {
              ...
              // Use delayed refetch instead of immediate refetch
              delayedRefetch();
              toast.success(t("notifications.categoryAdded"));
            }}
          />
        </div>
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <MoveSubCategoryModal
            ...
            categories={allCategories.filter(
              (cat) => cat.id !== subCategoryToMove.parentId
            )}
            onClose={() => {
              setShowMoveSubCategoryModal(false);
              setSubCategoryToMove(null);
