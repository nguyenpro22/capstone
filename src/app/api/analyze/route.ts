...
function analyzeBeautyServices(face: protos.google.cloud.vision.v1.IFaceAnnotation) {
    const {
      joyLikelihood,
      sorrowLikelihood,
      angerLikelihood,
    ...
    if (sorrowLikelihood === "VERY_LIKELY" || sorrowLikelihood === "LIKELY") {
      services.push("Chăm sóc da thư giãn", "Mát xa mặt", "Liệu pháp phục hồi da");
    } 
    ...
    if (headwearLikelihood === "VERY_LIKELY" || headwearLikelihood === "LIKELY") {
      services.push("Tư vấn kiểu tóc phù hợp với phụ kiện", "Chăm sóc tóc chuyên sâu");
    }
    ...
    if (
      ...
      if (faceWidth / faceHeight > 1) {
        services.push("Tạo khối giúp khuôn mặt thon gọn hơn", "Tư vấn kiểu tóc dài giúp cân đối khuôn mặt");
      } else {
        services.push("Highlight để làm nổi bật nét mềm mại trên khuôn mặt", "Tư vấn tạo kiểu tóc bồng bềnh");
      }
    }
